...
export class HomeService {
  ...
  getOrders(statusOrder: string) {
    ...
    const payload = new HttpParams()
      .set('startDate', '2023-04-01 00:00:00')
      .set('endDate', '2023-06-30 23:59:59')
      .set('statusOrder', statusOrder);
    const headers = new HttpHeaders().set('Authorization', `Bearer ${localStorage.getItem('token')}` || '');
    return this.http.get<BillInterface>(`${this._urlBackendApi}/bill/list?${payload}`, {
      headers,
    });
  }
  setStatusOrder(idBill: number, statusOrder: string) {
    ...
    const headers = new HttpHeaders().set('Authorization', `Bearer ${localStorage.getItem('token')}` || '');
    return this.http.get<OrderStatus>(`${this._urlBackendApi}/orders/status/${idBill}?${payload}`, {
      headers,
    });
  }
}
