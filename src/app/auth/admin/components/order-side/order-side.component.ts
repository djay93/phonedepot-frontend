    ...
    switch (this.statusOrder) {
      case 'NEW':
        Swal.fire({
          ...
          text: 'The status will be changed to "Processing"',
          icon: 'warning',
          ...
          if (result.isConfirmed) {
            this.homeService.setStatusOrder(idBill, 'PROCESSING').subscribe(() => {
              this.getOrders('NEW');
            });
            Swal.fire('Success!', 'Look into the Processing section for the order #' + idBill, 'success');
          }
        });
