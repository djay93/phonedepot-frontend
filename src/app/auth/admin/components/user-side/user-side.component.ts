...
export class UserSideComponent implements OnInit, OnChanges {
  ...
  constructor(private formBuilder: FormBuilder, private userService: UserService, private userPage: UserComponent) {}
  ...
  clean() {
    this.user.controls['password'].addValidators([Validators.required, Validators.minLength(6)]);
  ...
  editUsers() {
    this.alterableUser = this.user.value;
    this.alterableUser.phone = 0;
    this.userService.editUsers(this.alterableUser, this.editUser.idUser!).subscribe(() => {
      this.userPage.ngOnInit();
      this.clean();
    });
  }
